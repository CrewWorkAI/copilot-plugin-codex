...
The transcript is JSONL when the job used `--output-format=json` (rescue), otherwise plain text. If the job is still running, tell the user so and point them at `/copilot:status --wait <job-id>` rather than printing a partial transcript.
