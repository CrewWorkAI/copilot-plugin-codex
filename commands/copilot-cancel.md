...
Note: cancellation doesn't create an additional premium request, but it also doesn't refund the request already consumed by the running job. Any partial work Copilot did up to that point is preserved in the transcript — point the user at `/copilot:result <job-id>` if they want to see what was produced before the cancel.
