...
# `$copilot-plugin-codex cancel <job-id>` / `/copilot:cancel <job-id>` — terminate a background job
...
Any partial output Copilot emitted before the SIGTERM is preserved in the transcript file. Point the user at `$copilot-plugin-codex result <job-id>` or `/copilot:result <job-id>` if they want to see what was produced before the cancel.
...
- Process already exited → script reports this; metadata is still flipped to `cancelled`. Tell the user the job had already finished and offer `$copilot-plugin-codex result` or `/copilot:result`.
